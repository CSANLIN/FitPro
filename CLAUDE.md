...
> **注意：写日志任务（开发日志和学习日志）必须停下来提醒用户，等待用户下一步指示。只有写记忆（memory）不用停。**
...
3. **测试用例确认与编写**
   - **不要自动执行测试用例**，测试是用户的责任
   - **停下来询问用户**："代码编写完成，是否需要为此阶段编写测试用例？"
   - 等待用户确认后，才在 `docs/test_guide.md` 中追加新的测试用例
   - **写测试用例时只需追加**，不要重新读取整个文档（使用 `cat >>` 或 `Bash` 追加）
   - 测试用例应包含：测试目的、步骤、预期结果、测试结果记录区域
4. **写入开发日志 `docs/dev_log.md`**
   - 格式见下方"开发日志规范"
   - 每个二级任务完成后（如2.1、2.2…）写一条记录，三级任务（如2.3.1、2.3.2…）完成时不用写
   - **开始写开发日志前要停下来提醒用户**，等待用户确认后再继续
   - 写完开发日志你需要切换Haiku4.5模型继续写入学习日志
5. **写入学习日志 `docs/learning_log.md（到这一步你要停下来，提醒我换模型haiku）`**
   - **开始写学习日志前要停下来提醒用户**，等待用户切换模型并确认后再继续
...
1. **编写时机**：每个 Phase 完成后，在写开发日志之前
2. **交互流程**：
   - 代码编写完成后，**停下来询问用户**："代码编写完成，是否需要为此阶段编写测试用例？"
   - 等待用户确认后，才编写测试用例
   - **不要自动执行测试用例**，测试是用户的责任
3. **文件操作**：
   - **不要重新读取整个 `docs/test_guide.md`** 文件
   - 使用 `cat >> docs/test_guide.md` 或 `Bash` 命令追加新内容
   - 只需要追加新的测试用例，不需要修改现有内容
4. **测试用例格式**：
   ```markdown
   ### X.X 功能名称测试
   **测试目的**：验证...功能是否正常工作
...
1. **修复触发**：
   - **不要自动修复 bug**，等待用户主动告知："我的测试文档里面的测试结果已经写好了，帮我修复"
   - 用户提供具体的测试结果和问题描述
   
2. **修复后记录**：
   
   - 修复完成后，在 `docs/test_guide.md` 末尾追加修复记录
   - 使用 `cat >>` 追加，不要重新读取整个文档
   - **追加完成后打断点**，停止继续处理，等待用户下一步指示
...
3. **修复记录格式**：
   ...
   **验证方法**：重新执行测试用例 X.X，确认问题已解决
   ```
   
4. **文档更新**：
   - 修复完成后，同步更新 `docs/dev_log.md` 和 `docs/learning_log.md`
   - 遵循正常的开发工作流规范（包括询问用户确认）
