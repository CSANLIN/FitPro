...
│   ├── [ ] Vue Router 基础路由
│   ├── [ ] Pinia 状态管理
│   ├── [ ] Axios 封装 (请求/响应拦截器, Token 注入)
│   ├── [ ] 全局样式 / CSS 变量
│   └── [ ] 基础布局组件 (AdminLayout / AppLayout)
└── 1.5 联调验证
    ├── [ ] 后端 Knife4j 文档可访问
    ├── [ ] 前端请求后端 /api/health 接口成功
    └── [ ] CORS 跨域配置验证
```
...
Phase 2 - 认证体系
├── 2.1 后端安全
│   ├── [ ] SecurityConfig.java (放行白名单, 角色配置)
│   ├── [ ] JwtTokenProvider.java (生成/解析/刷新 Token)
│   ├── [ ] JwtAuthFilter.java (请求拦截, Token 校验)
│   ├── [ ] UserDetailsServiceImpl.java (加载用户信息)
│   ├── [ ] RedisConfig.java (Token 存储)
│   └── [ ] CorsConfig.java
├── 2.2 认证接口
