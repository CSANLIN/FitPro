...
1. **扮演新用户**：打开浏览器无痕窗口，访问 `http://localhost:5173/register`，注册一个新账号，然后登录，观察页面跳转。
2. **扮演“好奇”的会员**：用普通会员账号登录后，在浏览器地址栏手动输入 `/admin/dashboard` 并回车，观察是否出现403页面。
3. **观察Token刷新**：登录后，打开浏览器开发者工具 → 网络（Network）标签页，找到任意一个API请求，查看其请求头中的 `Authorization: Bearer xxxx`。等待2小时（或手动修改本地存储的token使其过期），再次触发一个API请求，观察网络请求中是否会先出现一个 `POST /api/auth/refresh` 的请求，然后原请求被重试。
...
✅ **流程完整性测试**
1. 注册新用户 → 登录 → 访问 `/api/auth/me` → 成功返回用户信息。
2. 退出登录 → 尝试访问 `/api/auth/me` → 被重定向到登录页。

✅ **角色权限测试**
1. 用超级管理员账号登录 → 访问 `/admin/dashboard` → 成功进入管理后台。
2. 用普通会员账号登录 → 访问 `/app/profile` → 成功进入会员中心。
3. 用普通会员账号登录 → 访问 `/admin/dashboard` → 显示403错误页面，并有“返回首页”按钮。

✅ **Token刷新测试**
1. 登录后，在浏览器控制台执行 `localStorage.setItem('token', 'eyJ...过期token...')` 手动使token过期。
2. 触发任意一个需要认证的API请求（如刷新页面）。
3. 观察网络请求中是否先有 `POST /api/auth/refresh`，然后原请求成功返回。

✅ **异常情况测试**
1. 修改Refresh Token为错误值 → 触发刷新 → 应跳转登录页。
2. 在弱网环境下，同时发起多个请求且Token过期 → 观察是否出现多个刷新请求（竞态条件）。
